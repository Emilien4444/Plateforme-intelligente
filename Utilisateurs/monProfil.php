...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
    ...
    $fileType = mime_content_type($_FILES['profile_picture']['tmp_name']); // Vérifie le type du fichier
    ...
    if (!in_array($fileType, $allowedTypes)) {
        ...
        $uploadDir = __DIR__ . "uploads/"; // Dossier où les fichiers seront stockés
        ...
        if (move_uploaded_file($_FILES['profile_picture']['tmp_name'], $uploadPath)) {
            ...
            if ($stmt->execute()) {
                // Maj réussie, afficher un message de succès
                $profile_picture = 'uploads/' . $fileName; // Mise à jour du chemin d'accès à l'image
        ...
        // Gestion des erreurs liées à l'upload (code d'erreur de l'upload)
        if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] !== UPLOAD_ERR_OK) {
            $message = '<div class="alert alert-danger text-center">Une erreur est survenue lors de l\'upload de la photo. Code d\'erreur: ' . $_FILES['profile_picture']['error'] . '</div>';
        }
    }
        ...
        <div class="card shadow-lg p-4">
            ...
            <div class="card shadow p-3 mb-4">
                <div class="mb-3">
                    ...
                    <?php if ($userLevel !== 'expert'): ?>
                        <p><strong>Pour passer au niveau Expert :</strong> Vous avez besoin de <strong>100 points</strong>.</p>
                        <p>Si vous souhaitez obtenir ces points plus rapidement, vous pouvez vous abonner à <strong>ePlatform+</strong> et bénéficier d'avantages exclusifs.</p>

                        <!-- Bouton pour accéder à la page de paiement -->
                        <a href="payment.php" class="btn btn-primary w-100">Abonnez-vous à ePlatform+ pour accélérer votre progression</a>
                    <?php endif; ?>
                </div>
