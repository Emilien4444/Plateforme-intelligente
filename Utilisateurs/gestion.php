...
// Récupérer les statistiques de consommation des appareils uniquement pour l'utilisateur connecté
$sqlStats = "SELECT device_id, SUM(consumption) AS total_consumption, AVG(current_temperature) AS avg_temperature 
             FROM consumption_stats 
             WHERE user_id = ? 
             GROUP BY device_id";

$stmtStats = $conn->prepare($sqlStats); 
$stmtStats->bind_param("i", $userId); // Lie l'ID de l'utilisateur à la requête
$stmtStats->execute(); 
                    ...
                    <tbody>
                        ...
                        <tr v-for="device in devices" :key="device.id">
                            ...
                            <td>
                                ...
                                <?php else: ?>
                                    <!-- Message indiquant que l'utilisateur n'a pas accès -->
                                    <span>Accumulez plus de points pour avoir accès à cette fonctionnalité.</span>
                                <?php endif; ?>
                            </td>
                        </tr>
                    </tbody>
